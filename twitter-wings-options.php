...
// Get Instance
if (isset($_GET['instance']) && (int)$_GET['instance']) {
	$inst = $_GET['instance'];
} else {
	$inst = 0;
}
...
if (isset($_GET['delete']) && (int)$_GET['delete']) {
	$twitter_wings->tw_delete_inst($_GET['delete']);
	$instances = get_option('tw_instances');
	unset($instances[$_GET['delete']]);
	update_option('tw_instances', $instances);
}
...
if (isset($_POST['save_options'])) {
	...
	$location = "options-general.php?page=twitter-wings&instance=$inst"; // based on the location of your sub-menu page
	...
	$instances[$inst] = 1;
	update_option('tw_instances', $instances);
	update_option("twitter_wings_options_$inst", $_POST);
	update_option('tw_oauth', array('consumerKey' => $_POST['consumerKey'],
	...
	// Delete cache
	delete_transient("tw_tweet_cache_$inst");
...
if (!$options) {
	$options = $twitter_wings->tw_install($inst);	
}

$oauth_options = get_option('tw_oauth');
$out = null;
$oauth = null;

// GET RATE LIMIT STATUS
if ($oauth_options['consumerKey'] && $oauth_options['consumerSecret'] && $oauth_options['accessToken'] && $oauth_options['accessTokenSecret']) {				
	$oauth = true;
	$oauth_message = true;
	require_once('twitteroauth/twitteroauth.php');
	$twitteroauth = new TwitterOAuth($oauth_options['consumerKey'], $oauth_options['consumerSecret'], $oauth_options['accessToken'], $oauth_options['accessTokenSecret']);
	$tw_limit = $twitteroauth->get('account/rate_limit_status');
					
	if ($tw_limit->hourly_limit == 150) {
		$oauth_message = false;
	}
}

if (!$oauth) {
	$json = @file_get_contents('https://api.twitter.com/1/account/rate_limit_status.json');
...
$title = __('Twitter Wings Pro', 'twitter-wings'); ?>

<div id="twitter-wings-admin" class="tw-admin wrap">
...
<form method="post" name="twitter_wings" id="twitter_wings_form">
	...
	<div class="admin-boxex">
	
		<div class="tw-oauth-admin tw-admin-box">
			
			<h3><?php _e('Twitter OAuth Settings', 'twitter-wings'); ?></h3>
			
			<?php if ($oauth && !$oauth_message) { ?>
				<p class="warning"><?php _e('Twitter Wings could not successfully connect to the the Twitter API using your application credentials. Please double-check that you entered them correctly.', 'twitter-wings'); ?></p>
			<?php } ?>
					
			<label for="consumerKey"><?php _e('Consumer key:', 'twitter-wings'); ?></label>
			<input type="password" id="consumerKey" name="consumerKey" class="tw_input" value="<?php echo $oauth_options['consumerKey']; ?>" />
			
			<label for="consumerSecret"><?php _e('Consumer secret:', 'twitter-wings'); ?></label>
			<input type="password" id="consumerSecret" name="consumerSecret" class="tw_input" value="<?php echo $oauth_options['consumerSecret']; ?>" />

			<label for="accessToken"><?php _e('Access token:', 'twitter-wings'); ?></label>
			<input type="password" id="accessToken" name="accessToken" class="tw_input" value="<?php echo $oauth_options['accessToken']; ?>" />

			<label for="accessTokenSecret"><?php _e('Access token secret:', 'twitter-wings'); ?></label>
			<input type="password" id="accessTokenSecret" name="accessTokenSecret" class="tw_input" value="<?php echo $oauth_options['accessTokenSecret']; ?>" />
		...
		<div class="tw-rate-limit tw-admin-box">
			...
			<h3><?php _e('Rate Limit Status', 'twitter-wings'); ?></h3>
			...
			<?php if ($tw_limit) {
				...
				$rate_source = ($oauth_message) ? 'application' : 'server';
				...
				$out .= '<p style="margin-bottom: 0;"><span class="desc">This ' . $rate_source . ' has accessed Twitter\'s API ' . ($tw_limit->hourly_limit - $tw_limit->remaining_hits) . ' times this hour. The rate limit will be reset in ' . $re_time_m . ' min, ' . $re_time_s . "s.</span></p>";
			}
		...
		<?php if (get_option("twitter_wings_options_$inst")) { ?>
		
			<div class="tw-feed tw-admin-box">		
				<?php TwitterWings(array('inst' => $inst,
									'before_title' => '<h3>', 
									'after_title' => '</h3>', 
	...
	<div class="tw-wrap-wrap">
		<div class="nav-tabs-wrapper">
				
			<div>				
				<?php $instances = get_option('tw_instances');
				$compare = 0;
			
				foreach($instances as $key => $instance) {
								
					$opt = get_option("twitter_wings_options_$key"); 
									
					$current = ($key == $inst) ? ' class="tw-current"' : ''; ?>
									
					<a href="/wp-admin/options-general.php?page=twitter-wings&instance=<?php echo $key ?>"<?php echo $current; ?>><?php echo stripslashes($opt['tw_inst_name']); ?></a>
				...
				<a href="/wp-admin/options-general.php?page=twitter-wings&instance=<?php echo $compare ?>" class="new-tw" title="Add new instance">&#43;</a>
			</div>		
		</div>
		<a href="#" class="tw-prev">&#171;</a>
		<a href="#" class="tw-next">&#187;</a>
	</div>
	
	<div class="tw-form-body">

		<?php wp_nonce_field('twitter_wings'); ?>
				
		<?php if ($inst != 0) { ?>
			<a href="/wp-admin/options-general.php?page=twitter-wings&delete=<?php echo $inst ?>" class="delete-tw">Delete instance</a>
		<?php } ?>
			
		<h3><?php _e('General Settings', 'twitter-wings'); ?></h3>
				
		<p class="formrow">
			<label for="usernames"><?php _e('Instance name:', 'twitter-wings'); ?></label>
			<input type="text" id="inst_name" name="tw_inst_name" class="tw_input" value="<?php echo stripslashes($options['tw_inst_name']); ?>" />
			<span class="desc"><?php _e('Name this feed. You will select the feed by name in the widget settings', 'twitter-wings'); ?></span>
		</p>
		...
		<p style="padding-left: 0.75%;"><input type="submit" class="button button-primary button-main" name="save_options" value="<?php _e('Save Options', 'twitter-wings'); ?>" /></p>
	</div>
</form>
</div>